...
public class STables {
    public static final short S_TABLE_SIZE = 16;

    private static final short[][] s0TableShort = {
    ...
    private static short[][] s1TableShort = {
    ...
    public static byte getS0TableByte(int i, int j) throws CryptoException {
        if (i < 0 || i >= s0TableShort.length || j < 0 || j >= s0TableShort[i].length){
            throw new CryptoException("Index out of bound exception: (" + i + ", " + j + ")");
        }
        return (byte) (255 & s0TableShort[i][j]);
    }

    public static byte getS1TableByte(int i, int j) throws CryptoException {
        if (i < 0 || i >= s1TableShort.length || j < 0 || j >= s1TableShort[i].length){
            throw new CryptoException("Index out of bound exception: (" + i + ", " + j + ")");
        }
        return (byte) (255 & s1TableShort[i][j]);
    }
}
